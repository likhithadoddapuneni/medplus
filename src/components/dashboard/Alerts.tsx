...
export function SafetyAlert({ message }: { message: string }) {
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-2xl border-2 border-[#D8443C] bg-[#D8443C]/8"
    >
      <div className="flex items-center gap-3 px-4 py-3">
        <motion.span
          animate={{ opacity: [1, 0.3, 1] }}
          transition={{ repeat: Infinity, duration: 1 }}
          className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#D8443C] text-white"
        >
          <Warning size={18} weight="fill" />
        </motion.span>
        <div>
          <div className="font-mono text-[10px] font-semibold uppercase tracking-widest text-[#D8443C]">
