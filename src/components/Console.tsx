...
export function Console({ isActive = true, onStatusChange, onTitleChange }: ConsoleProps) {
  ...
  const run = useCallback(
    async (text: string) => {
      ...
      setRunId("");
      setCaseText(text);
      onTitleChange?.(text.trim().substring(0, 60) + (text.length > 60 ? "..." : ""));
      startedRef.current = true;
      const initialStatus = "Connecting to the agent board…";
      setStatusLine(initialStatus);
      ...
      try {
        ...
        setRunning(false);
        setStatusLine("");
        if (!error) onStatusChange?.("Done");
      }
    },
    [running, newRoom, consume, onTitleChange, onStatusChange, error]
  );
  ...
  return (
    <div className={`space-y-5 ${isActive === false ? "hidden" : ""}`}>
      {/* HITL overlay */}
      <AnimatePresence>
        {paused && (
          <HitlOverlay
        ...
        <div className="flex flex-col gap-2 rounded-3xl border border-navy/20 bg-cream-soft p-3 md:flex-row md:items-end">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-2xl bg-triage px-5 py-2.5 text-sm font-semibold text-white shadow-[0_4px_16px_rgba(216,68,60,0.35)] transition active:scale-[0.98] disabled:opacity-40"
            >
        ...
        <div className="mt-2 flex items-center gap-3 px-1">
          ...
          >
            <span
              ...
              style={{ background: newRoom ? "#0D4A45" : "#CCDFDA" }}
            >
              <span
    ...
    <div className="flex min-h-[46dvh] flex-col items-center justify-center rounded-3xl border border-dashed border-navy-line bg-cream-soft/40 px-6 py-10 text-center">
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-5 max-w-4xl w-full text-left">
        <div className="rounded-2xl border border-triage/20 bg-triage/[0.02] p-3.5 flex flex-col justify-between">
          ...
          <span className="font-mono text-[10px] text-triage/80 mt-3 font-semibold">{AGENT_UI.triage.model}</span>
        </div>
        ...
        <div className="rounded-2xl border border-mgmt/20 bg-mgmt/[0.02] p-3.5 flex flex-col justify-between">
          ...
          <span className="font-mono text-[10px] text-mgmt/80 mt-3 font-semibold">{AGENT_UI.management.model}</span>
        </div>
        ...
        <div className="rounded-2xl border border-navy/25 bg-navy/[0.02] p-3.5 flex flex-col justify-between">
          ...
          <span className="font-mono text-[10px] text-navy/60 mt-3 font-semibold text-navy/80">{AGENT_UI.investigation.model}</span>
        </div>
        ...
        <div className="rounded-2xl border border-doc/20 bg-doc/[0.02] p-3.5 flex flex-col justify-between">
          ...
          <span className="font-mono text-[10px] text-doc/80 mt-3 font-semibold">{AGENT_UI.documentation.model}</span>
        </div>
        ...
        <div className="rounded-2xl border border-observer/20 bg-observer/[0.02] p-3.5 flex flex-col justify-between">
          ...
          <span className="font-mono text-[10px] text-observer/80 mt-3 font-semibold">{AGENT_UI.observer.model}</span>
        </div>
      </div>
